        ...
        <thead>
          <tr>
            {columns.map((column, index) => (
              <th key={index} className="border border-[#d2626] px-4 py-2 text-left">
                {column}
              </th>
            ))}
          </tr>
          ...
          {currentRows.map((row, rowIndex) => (
            <tr key={rowIndex}>
              {columns.map((column, colIndex) => (
                <td key={colIndex} className={`border border-[#262626] px-4 py-2 ${rowIndex % 2 === 0 ? "bg-gray-100" :"bg-white"}`}>
                  {column === "Action" ? (
    <div className="flex gap-2">
      
     <div className='bg-[#D17C16] flex p-1  text-white rounded-[5px] w-[70px]'>
      <MdDelete size={20}/>
      <h1 className='text-sm'>Delete</h1>
      </div>
      <div className='bg-[#2563ea] flex gap-1 items-center justify-center p-1  text-white rounded-[5px] w-[70px]'>
      <FaEdit size={20}/>
