...
import AddAccessoriesItems from "./CreateAccessoriesProduct";
import CustomModal from "../../../common/CustomModal";
import CustomPopup from "../../../common/CustomPopup";
import { showToast } from "../../../common/CustomToast";

function AccessoriesProductList() {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [showDeletePopup, setShowDeletePopup] = useState(false);
  const [selectedRecord, setSelectedRecord] = useState(null);

  const { data: allItems = {}, refetch } = useGetAllAccessoriesItemsQuery();
  const [deleteAccessoriesItem, { isLoading: isDeleting }] =
    useDeleteAccessoriesItemsMutation();
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await deleteAccessoriesItem(selectedRecord.Id).unwrap();
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-[10px] shadow-md mt-8 pt-4 mb-20 flex items-center justify-center h-auto w-[90%] mx-auto">
        <div className="w-[90%]">
          <h1 className="text-lg font-[500] my-4">All Accessories Products</h1>
          <CustomTable
            rows={rows}
            columns={columns}
      ...
      {isModalOpen && (
        <CustomModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
          <AddAccessoriesItems
            selectRecordIndex={selectedRecord}
